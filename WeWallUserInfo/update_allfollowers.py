...
for i in range(5000):
    ...
    try:
        ...
        break
    for k in wx:
        cursor.execute("SELECT username from wewall_username where id = '%s';"%str(k['user_openid']))
        result = cursor.fetchone()
        username = k['user_name'].replace("'", "''")
        if not result:
            cursor.execute("INSERT INTO wewall_username (id, username, create_time) VALUES ('%s', '%s', '%s')" %(k['user_openid'], username, k['user_create_time']))
            db.commit()
        else:
            if k['user_name'] != result[0]:
                cursor.execute("UPDATE wewall_username SET username = '%s' WHERE id = '%s';"%(username, k['user_openid']))
                db.commit()
                print(username)
            else:
                print("ok")
